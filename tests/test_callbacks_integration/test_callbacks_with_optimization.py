...
def test_callbacks_during_optimization(mocked_lms, recorder_dir):
    ...
    for call in proposal_end_calls:
        assert "proposed_improvements" in call
        ...
        # prompts and raw_lm_outputs should have the same keys as proposed_improvements
        assert set(call["prompts"].keys()) == set(call["proposed_improvements"].keys())
        assert set(call["raw_lm_outputs"].keys()) == set(call["proposed_improvements"].keys())
        assert call["new_instructions"] == call["proposed_improvements"]
        for comp_name in call["proposed_improvements"]:
